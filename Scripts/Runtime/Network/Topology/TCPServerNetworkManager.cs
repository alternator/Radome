...
//		public NativeStreamWriter NativeStreamWriter = default;
//		public NativeList<ushort> DataStreamSizeList;
//		public NativeList<NativeStreamReader> DataStreamPackets;
...
//						if (state.NativeStreamWriter.IsCreated) state.NativeStreamWriter.Dispose();
...
//			{
//				Debug.Log("Failed to bind to port : " + IPEndPoint + " : " + e);
...
//		protected override unsafe void SendToConnIdImmediately(short connId, NativeStreamWriter packet, bool reliable)
//		{
//			if (connId == -1) return;
...
//					if (!state.NativeStreamWriter.IsCreated) state.NativeStreamWriter = new NativeStreamWriter(TcpStateObject.StateBufferSize, Allocator.Persistent);
//					if (!state.DataStreamSizeList.IsCreated) state.DataStreamSizeList = new NativeList<ushort>(32, Allocator.Persistent);
//					if (!state.DataStreamPackets.IsCreated) state.DataStreamPackets = new NativeList<NativeStreamReader>(32, Allocator.Persistent);

//					state.NativeStreamWriter.Clear();
...
//						{
//							state.NativeStreamWriter.WriteBytes(ptr, state.BufferStartIndex);
//						}
...
//			using (var packet = new NativeStreamWriter(9, Allocator.Temp))
//			{
...
//						managerId = ManagerId,
//						connections = _ConnectionIdList,
//						recieveDataStream = state.NativeStreamWriter,
...
//				var temp = new NativeStreamWriter(NetworkParameterConstants.MTU, Allocator.Temp);
...
//			public NativeStreamWriter recieveDataStream;
//			public NativeList<ushort> recieveDataSizeList;
//			public NativeList<NativeStreamReader> recieveDataPackets;
...
//			{
//				var manager = _TcpServers[managerId];
//				var relayWriter = new NativeStreamWriter(NetworkParameterConstants.MTU, Allocator.Temp);
//				var multiCastList = new NativeList<ushort>(Allocator.Temp);

//				int offset = 0;
//				for(int index=0; index < recieveDataSizeList.Length; index++)
//				{
//					var stream = new NativeStreamReader(recieveDataStream, offset, recieveDataSizeList[index]);
...
//						{
//							var c = new NativeStreamReader.Context();
...
//						var ctx2 = new NativeStreamReader.Context();
...
//			private void PurgeChunk(ushort senderPlayerId, ref NativeStreamReader stream, ref NativeStreamReader.Context ctx)
//			{
//				while (true)
//				{
...
//			private unsafe void RelayPacket(ushort targetPlayerId, NativeStreamReader stream, byte qos
//				, TCPServerNetworkManager<PlayerInfo> manager, ref NativeStreamWriter relayWriter)
//			{
//				relayWriter.Clear();
