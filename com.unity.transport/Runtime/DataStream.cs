...
namespace Unity.Networking.Transport
{
    ...
    public unsafe struct DataStreamWriter : IDisposable
    {
        ...
        public DeferredLong Write(long value)
        {
            var ret = new DeferredLong { m_writer = this, m_offset = m_Data->length + ((m_Data->bitIndex + 7) >> 3) };
            WriteBytes((byte*)&value, sizeof(long));
            return ret;
        }

        public DeferredULong Write(ulong value)
        {
            var ret = new DeferredULong { m_writer = this, m_offset = m_Data->length + ((m_Data->bitIndex + 7) >> 3) };
        ...
        public DeferredDouble Write(double value)
        {
            var ret = new DeferredDouble { m_writer = this, m_offset = m_Data->length + ((m_Data->bitIndex + 7) >> 3) };
            UIntFloat uf = new UIntFloat();
            uf.doubleValue = value;
            Write((int)uf.longValue);
            return ret;
        }
    ...
    public unsafe struct DataStreamReader
    {
        ...
        public DataStreamReader (DataStreamReader reader, int offset, int length) 
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            if (offset + length > reader.Length)
                throw new System.ArgumentOutOfRangeException ();
            m_Safety = reader.m_Safety;
#endif
            m_bufferPtr = reader.m_bufferPtr + offset;
            m_Length = length;
        ...
        public DataStreamReader ReadChunk (ref Context ctx, int length) {
            var chunk = new DataStreamReader (this, ctx.m_ReadByteIndex, length);
        ...
        {
            UIntFloat uf = new UIntFloat();
            uf.intValue = (uint) ReadUInt(ref ctx);
            return uf.floatValue;
        }

        public double ReadDouble(ref Context ctx)
        {
            UIntFloat uf = new UIntFloat();
            uf.doubleValue = (ulong) ReadULong(ref ctx);
            return uf.doubleValue;
        }

		public UnityEngine.Vector2 ReadVector2 (ref Context ctx) {
			return new UnityEngine.Vector2 (ReadFloat (ref ctx), ReadFloat (ref ctx));
		}

		public UnityEngine.Vector3 ReadVector3 (ref Context ctx) {
			return new UnityEngine.Vector3 (ReadFloat (ref ctx), ReadFloat (ref ctx), ReadFloat (ref ctx));
		}

		public UnityEngine.Vector4 ReadVector4 (ref Context ctx) {
			return new UnityEngine.Vector4 (ReadFloat (ref ctx), ReadFloat (ref ctx), ReadFloat (ref ctx), ReadFloat (ref ctx));
		}
